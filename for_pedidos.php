...
if(isset($_POST['cod_pedido']) && isset($_POST['cod_material']) ) {
	...
	if (isset($_POST['precio_compra'])) { $precio_compra = limpiar($_POST['precio_compra']); } else { $precio_compra = ''; }
...
mysql_query("select * from orden_compras where cod_material = '".$cod_material."' and rutprov != '".$rutprov."'") or die(mysql_error());
	...
	mysql_query("select * from pedidos where cod_material = '".$cod_material."' and fecha_pedido = '".$fecha_pedido."' and rutprov = '".$rutprov."'") or die(mysql_error());
			if (mysql_affected_rows() == 0) {
			
		mysql_query("insert into pedidos (cod_pedido,cod_material,material_pedido,cantidad_pedido,fecha_pedido,rutprov) values ('".mysql_real_escape_string($cod_pedido)."','".mysql_real_escape_string($cod_material)."','".mysql_real_escape_string($material_pedido)."','".mysql_real_escape_string($cantidad_pedido)."','".mysql_real_escape_string($fecha_pedido)."','".mysql_real_escape_string($rutprov)."')") or die(mysql_error());
		
    echo"<div class='alert alert-success'>";
	echo"<button type='button' class='close' data-dismiss='alert'>×</button>";
	echo"<center>EL PEDIDO AL PROVEEDOR CON RUF $rutprov FUE REGISTRADO SATISFACTORIAMENTE</div>";
	echo '<br>';
	...
	echo"<button type='button' class='close' data-dismiss='alert'>×</button>";
	echo"<center>ERROR! YA EXISTE UN PEDIDO REGISTRADO A ESTE PROVEEDOR Y FECHA</div>";
	echo '<br>';
 ...
 ?>
          <?php 
			$sql1 = mysql_query("select cod_pedido from pedidos order by cod_pedido desc") or die(mysql_error());
			$array1 = mysql_fetch_array($sql1);
			$n = $array1['cod_pedido'] + 1;
			...
			?>
                  <form id="form1" name="form1" class="form-horizontal" enctype="multipart/form-data" method="post" action="">
                    ...
                    <div class="control-group">
                      ...
                      <input name="cod_pedido" type="text" class="input-xlarge disabled" id="cod_pedido" size="10" value="<?php echo $num; ?>" readonly="readonly" />
                      </div>
                    </div>
					</div> 
                      <div class="control-group">
               ...
               <input name="material" type="text" class="input-xlarge focused" id="material"  onKeyUp="this.value=this.value.toUpperCase();" size="30" autocomplete="off" pattern="[A-ZÑa-zñ0-9º ]{3,50}" placeholder="Ingrese Descripcion Material" required="required"/>
                      </div>
                      </div>
					  </div>
					   <div class="control-group">
                      ...
                      <div class="controls">
					  <div class="input-prepend" title="Ingrese Precio del Producto" data-rel="tooltip">
                     <span class="add-on">$</span><input name="precio_compra" type="text" id="precio_compra" class="input-xlarge focused" pattern="[0-9.]{2,10}" placeholder="Ingrese Precio Compra C/U" autocomplete="off" onKeyUp="this.value=this.value.toUpperCase();" readonly="readonly" />
                      </div>
                    </div>
					</div>
                      <div class="control-group">
					  ...
					  <div class="input-prepend" title="Ingrese solo numeros para Cantidad" data-rel="tooltip">
                         <input name="cantidad_pedido" type="text" class="input-xlarge focused" id="cantidad_pedido" autocomplete="off"  pattern="[0-9]{1,5}" placeholder="Ingrese Cantidad" required="required"/>
                      </div>
                    </div>
					</div>
                      <div class="control-group">
...
?>
					...
					<div class="control-group">
                      ...
                      <div class="controls">
					  <div class="input-prepend" title="Codigo Orden de Entrada" data-rel="tooltip">
                      <input name="fecha_pedido" type="text" class="input-xlarge disabled" id="fecha_pedido" size="10" value="<?php echo date('Y-m-d'); ?>" readonly="readonly" />
                      </div>
                    </div>
					</div> 
