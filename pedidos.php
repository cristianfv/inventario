...
<li><a class="ajax-link" href="configuracion.php"><i class="icon-wrench"></i><span class="hidden-tablet"> Configuraci&oacute;n</span></a></li>
<li><a class="ajax-link" href="logout.php"><i class="icon-off"></i><span class="hidden-tablet"> Cerrar Sesi&oacute;n</span></a></li>
</ul>
</div><!--/.well -->
			</div><!--/span-->
			<!-- left menu ends -->
			...
			<div class="row-fluid sortable">
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i> Consulta General de Pedidos de Productos a Proveedores </h2>
						<div class="box-icon"> 
<a href="#" class="btn btn-minimize btn-round" title="Desplegar" data-rel="tooltip"><i class="icon-chevron-up"></i></a>
	...
	<?php 
	   ...
	   if (isset($_GET['eliminar'])) {
		...
		$cod_pedido = base64_decode($_GET['cod_pedido']); 
		
		
		if ($elimina == 'eliminar_pedido' && $_SESSION['acceso'] == "admin") {
		
		mysql_query("delete from pedidos where cod_pedido = '".mysql_real_escape_string($cod_pedido)."'") or die(mysql_error());
			
		echo '<br>';
		echo"<div class='alert alert-block'>";
	    echo"<button type='button' class='close' data-dismiss='alert'>×</button>";
	    echo"<strong>EL PEDIDO N° $cod_pedido HA SIDO ELIMINADO SATISFACTORIAMENTE</strong></div>";
	   ...
	   echo"<button type='button' class='close' data-dismiss='alert'>×</button>";
	   echo"<strong>TU NO ERES ADMINISTRADOR, NO TIENES PERMISOS PARA ELIMINAR PEDIDOS</strong></div>";
	   ...
	   ?>				
					<div class="box-content">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  <thead>
							  <tr>
							    <th width="10%"><div align="center">C&oacute;digo</div></th>
							    <th width="30%"><div align="center">Producto</div></th>
							    <th width="10%"><div align="center">Cantidad</div></th>
							    <th width="12%"><div align="center">Fecha</div></th>
							    <th width="15%"><div align="center">Proveedor</div></th>
							    <th width="23%"><div align="center">Acciones</div></th>
							  </tr>
						  </thead>   
						  <tbody>
						  <?php 
$sql = mysql_query("SELECT * FROM pedidos ") or die(mysql_error());
while($array=mysql_fetch_array($sql)) {  
?>
							<tr>
								<td><div align="center"><?php echo $array["cod_material"]; ?></div></td>
								<td class="center"><div align="center"><?php echo $array["material_pedido"]; ?></div></td>
								<td class="center"><div align="center"><?php echo $array["cantidad_pedido"]; ?></div></td>
								<td class="center"><div align="center"><?php echo $array["fecha_pedido"]; ?></div></td>
								<td class="center"><div align="center"><?php 
$sql1 = mysql_query("select * from proveedores where rutprov = '".$array['rutprov']."'") or die("Problemas en el select:".mysql_error());
$regi=mysql_fetch_array($sql1);

								echo '<a href="#" title="'.$regi['nombreprov'].'" data-rel="tooltip">'.$array["rutprov"].'</a>';?></div></td>
								<td class="center">
<a class="btn btn-info ask" href="modifica_pedidos.php?cod_pedido=<?php echo base64_encode($array['cod_pedido']); ?>" title="Modificar Pedido" data-rel="tooltip" onClick="return confirm('&iquest;Esta seguro que desea Modificar los datos de este Pedido?')"><i class="icon-edit icon-white"></i>Edita</a>
<a class="btn btn-danger" href="pedidos.php?eliminar=<?php echo base64_encode('eliminar_pedido'); ?>&cod_pedido=<?php echo base64_encode($array['cod_pedido']); ?>" title="Eliminar Pedido" data-rel="tooltip" onClick="return confirm('&iquest;Esta seguro que desea Eliminar este Pedido?')"><i class="icon-trash icon-white"></i>Elimina</a>									</td>
							</tr><?php } ?>
						  </tbody>
					  </table>            
					</div>
