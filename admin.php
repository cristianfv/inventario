...
<li><a class="ajax-link btn-setting" href="perfil.php" ><i class="icon-user" btn-setting></i><span class="hidden-tablet"> Mi Perfil</span></a></li>
						<li class="divider"></li>
...
<li><a class="ajax-link" href="logout.php"><i class="icon-off"></i><span class="hidden-tablet"> Cerrar Sesión</span></a></li>
			...
			<div>
				<ul class="breadcrumb">
					<li>
						<i class="icon-home"></i><a href="admin.php"> Principal</a>
					</li>
				</ul>
			</div><?php 
$sql = mysql_query("select count(*) as total from usuarios") or die(mysql_error());
$re=mysql_fetch_array($sql);

$sql2 = mysql_query("select count(*) as total from orden_compras") or die(mysql_error());
$re2=mysql_fetch_array($sql2);

$sql3 = mysql_query("select count(*) as total from material_almacen") or die(mysql_error());
$re3=mysql_fetch_array($sql3);

$sql4=mysql_query("select count(*) as total from material_almacen where existencia <= minimo_stock") or die("Problemas en el select:".mysql_error());
$re4=mysql_fetch_array($sql4);	
?>
			<div class="sortable row-fluid">
	<a data-rel="tooltip" title="<?php echo $re['total']." Usuarios Registrados"; ?>" class="well span3 top-block" href="#">
					<span class="icon32 icon-red icon-user"></span>
					<div>Total Usuarios</div>
					<div><?php echo $re['total']; ?></div>
					<span class="notification"><?php echo $re['total'];	?></span>
				</a>

				<a data-rel="tooltip" title="<?php echo $re2['total']." Ordenes de Compras"; ?>" class="well span3 top-block" href="#">
					<span class="icon32 icon-color icon-star-on"></span>
					<div>Orden de Compras</div>
					<div><?php echo $re2['total']; ?></div>
					<span class="notification green"><?php echo $re2['total']; ?></span>
				</a>

				<a data-rel="tooltip" title="<?php echo $re3['total']." Materiales en Almacen"; ?>" class="well span3 top-block" href="#">
					<span class="icon32 icon-color icon-cart"></span>
					<div>Material en Almacen</div>
					<div><?php echo $re3['total'];	?></div>
					<span class="notification yellow"><?php echo $re3['total'];	?></span>
				</a>
				
				<a data-rel="tooltip" title="<?php echo $re4['total']." Materiales en Stock Minimo"; ?>" class="well span3 top-block" href="#">
					<span class="icon32 icon-blue icon-archive"></span>
					<div>Material Minimo Stock </div>
					<div><?php echo $re4['total'];	?></div>
					<span class="notification red"><?php echo $re4['total'];	?></span>
				</a>
			</div>
			...
			<div class="row-fluid">
				<div class="box span12">
					<div class="box-header well">
						<h2><i class="icon-info-sign"></i> Introducción</h2>
						<div class="box-icon">
<a href="#" class="btn btn-minimize btn-round" title="Desplegar" data-rel="tooltip"><i class="icon-chevron-up"></i></a>
						</div>
					</div>
					<div class="box-content">
						<h2>Sistema de Registro y Control de Inventarios </h2><br>
						<p align="justify">El   inventario es uno de los factores principales que determinan cómo   trabajan las empresas y obtienen beneficios; es saludable e importante   para todas las empresas el operar y administrar bien sus inventario. El   enfoque y el objetivo del control de inventarios es mantener un nivel   óptimo del inventario y su inversión</p>
