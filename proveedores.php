			...
			<div class="row-fluid sortable">
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i> Consulta General de Proveedores </h2>
						<div class="box-icon"> 
<a href="#" class="btn btn-minimize btn-round" title="Desplegar" data-rel="tooltip"><i class="icon-chevron-up"></i></a>
	...
	<?php 
	   ...
	   if (isset($_GET['eliminar'])) {
		...
		$rutprov = base64_decode($_GET['rutprov']); 
		
		
		if ($elimina == 'eliminar_proveedor' && $_SESSION['acceso'] == "admin") {
		
		mysql_query("select * from orden_compras where rutprov = '".$rutprov."'") or die(mysql_error());
		if (mysql_affected_rows() == 0) {
		
		mysql_query("delete from proveedores where rutprov = '".mysql_real_escape_string($rutprov)."'") or die(mysql_error());
			
		echo '<br>';
		echo"<div class='alert alert-block'>";
	    echo"<button type='button' class='close' data-dismiss='alert'>×</button>";
	    echo"<strong>EL PROVEEDOR HA SIDO ELIMINADO SATISFACTORIAMENTE</strong></div>";
		...
		echo"<div class='alert alert-block'>";
	    echo"<button type='button' class='close' data-dismiss='alert'>×</button>";
	    echo"<strong>NO PUEDE SER ELIMINADO, ESTE PROVEEDOR ESTA RELACIONADO CON PRODUCTOS EXISTENTES</strong></div>";
		}
	   ...
	   echo"<button type='button' class='close' data-dismiss='alert'>×</button>";
	   echo"<strong>TU NO ERES ADMINISTRADOR, NO TIENES PERMISOS PARA ELIMINAR PROVEEDORES</strong></div>";
	   ...
	   ?>				
					<div class="box-content">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  <thead>
							  <tr>
							    <th width="13%"><div align="center"><span data-reactid=".zg.$mid=11434258871829=2796879f53f0f46fb68.2:0.0.0.0.0"><span data-reactid=".zg.$mid=11434258871829=2796879f53f0f46fb68.2:0.0.0.0.0.0">RFC</span></span></div></th>
							    <th width="26%"><div align="center">Nombres</div></th>
							    <th width="10%"><div align="center">Telefono</div></th>
							    <th width="27%"><div align="center">Direcci&oacute;n</div></th>
							    <th width="24%"><div align="center">Acciones</div></th>
							  </tr>
						  </thead>   
						  <tbody>
						  <?php 
$sql = mysql_query("SELECT * FROM proveedores ") or die(mysql_error());
while($array=mysql_fetch_array($sql)) {  
?>
							<tr>
								<td><div align="center"><?php echo $array["rutprov"]; ?></div></td>
								<td class="center"><div align="center"><?php echo $array["nombreprov"]; ?></div></td>
								<td class="center"><div align="center"><?php echo $array["telefprov"]; ?></div></td>
								<td class="center"><div align="center"><?php echo $array["direcprov"]; ?></div></td>
								<td class="center">
<a class="btn btn-info ask" href="modifica_proveedor.php?cod_proveedor=<?php echo base64_encode($array['cod_proveedor']); ?>" title="Modificar Proveedor" data-rel="tooltip" onClick="return confirm('&iquest;Esta seguro que desea Modificar los datos del Proveedor?')"><i class="icon-edit icon-white"></i>Edita</a>
<a class="btn btn-danger" href="proveedores.php?eliminar=<?php echo base64_encode('eliminar_proveedor'); ?>&rutprov=<?php echo base64_encode($array['rutprov']); ?>" title="Eliminar Proveedor" data-rel="tooltip" onClick="return confirm('&iquest;Esta seguro que desea Eliminar este Proveedor?')"><i class="icon-trash icon-white"></i>Elimina</a>									</td>
							</tr><?php } ?>
						  </tbody>
					  </table>            
					</div>
