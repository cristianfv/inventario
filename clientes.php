			...
			<div class="row-fluid sortable">
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i> Consulta General de Clientes </h2>
						<div class="box-icon"> 
<a href="#" class="btn btn-minimize btn-round" title="Desplegar" data-rel="tooltip"><i class="icon-chevron-up"></i></a>
	...
	<?php 
	   ...
	   if (isset($_GET['eliminar'])) {
		...
		if ($elimina == 'eliminar_cliente' && $_SESSION['acceso'] == "admin") {
		
		mysql_query("delete from clientes where rutcliente = '".mysql_real_escape_string($rutcliente)."'") or die(mysql_error());
		mysql_query("delete from ventas where rutcliente = '".mysql_real_escape_string($rutcliente)."'") or die(mysql_error());
		mysql_query("delete from detalle_ventas where rutcliente = '".mysql_real_escape_string($rutcliente)."'") or die(mysql_error());
			
		echo '<br>';
		echo"<div class='alert alert-block'>";
	    echo"<button type='button' class='close' data-dismiss='alert'>×</button>";
	    echo"<strong>EL CLIENTE HA SIDO ELIMINADO SATISFACTORIAMENTE</strong></div>";
	   ...
	   echo"<button type='button' class='close' data-dismiss='alert'>×</button>";
	   echo"<strong>TU NO ERES ADMINISTRADOR, NO TIENES PERMISOS PARA ELIMINAR CLIENTES</strong></div>";
						...
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  <thead>
							  <tr>
							    ...
							    <th width="21%"><div align="center">Acciones</div></th>
							  </tr>
						  </thead>   
						  <tbody>
						  <?php 
...
?>
							<tr>
								<td><div align="center"><?php echo $array["rutcliente"]; ?></div></td>
								<td class="center"><div align="center"><?php echo $array["nombrecliente"]; ?></div></td>
								<td class="center"><div align="center"><?php echo $array["telefcliente"]; ?></div></td>
								<td class="center"><div align="center"><?php echo $array["direccliente"]; ?></div></td>
								<td class="center">
<a class="btn btn-info ask" href="modifica_cliente.php?cod_cliente=<?php echo base64_encode($array['cod_cliente']); ?>" title="Modificar Cliente" data-rel="tooltip" onClick="return confirm('&iquest;Esta seguro que desea Modificar los datos del Cliente?')"><i class="icon-edit icon-white"></i>Edita</a>
<a class="btn btn-danger" href="clientes.php?eliminar=<?php echo base64_encode('eliminar_cliente'); ?>&rutcliente=<?php echo base64_encode($array['rutcliente']); ?>" title="Eliminar Cliente" data-rel="tooltip" onClick="return confirm('&iquest;Esta seguro que desea Eliminar este Cliente?')"><i class="icon-trash icon-white"></i>Elim</a>									</td>
							</tr><?php } ?>
						  </tbody>
					  </table>            
					</div>
