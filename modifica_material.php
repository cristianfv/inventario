...
var minimo=parseInt(document.getElementById('minimo_stock').value);
var actual=parseInt(document.getElementById("existencia").value);

    console.log(minimo_stock+ "  "+ existencia)
if (actual < minimo){
   alert('ERROR! LA EXISTENCIA ' + actual + ' NO PUEDE SER MENOR AL STOCK MINIMO '+ minimo);
  return(false);
}
}
...
<li><a class="ajax-link" href="logout.php"><i class="icon-off"></i><span class="hidden-tablet"> Cerrar Sesión</span></a></li>
			...
			<i class="icon-home"></i><a href="admin.php"> Principal</a> <span class="divider">/</span>
					...
					<li>
				<i class="icon-tasks"></i><a href="almacen.php"> Produtos en Almacen</a><span class="divider">/</span></li>
					<li>
					<li>
			<i class="icon-print"></i><a href="reporte_materiales.php" target="_blank"> Listado de Productos en Almacen</a></li>
				</ul>
			</div>
			...
			<div class="row-fluid sortable">
                ...
                <div class="box-header well" data-original-title>
                  <h2><i class="icon-edit"></i> Formulario de Actualización de Productos en Almacen </h2>
                  <div class="box-icon">  
...
if(isset($_POST['cod_orden']) && isset($_POST['material']) ) {
	...
	$minimo_stock= limpiar($_POST['minimo_stock']);
	$ubicacion_deposito= limpiar($_POST['ubicacion_deposito']);
	$status_material = limpiar($_POST['status_material']);
...
mysql_query("update material_almacen set minimo_stock = '".mysql_real_escape_string($minimo_stock)."',ubicacion_deposito = '".mysql_real_escape_string($ubicacion_deposito)."',status_material = '".mysql_real_escape_string($status_material)."' where cod_orden = '".$cod_orden."'") or die(mysql_error());
	
		
	echo"<div class='alert alert-block'>";
	echo"<button type='button' class='close' data-dismiss='alert'>×</button>";
	echo"<center>EL PRODUCTO $material FUE ACTUALIZADO SATISFACTORIAMENTE</div>";
	echo "<meta http-equiv='Refresh' content='3;url=almacen.php'>"; 
	echo '<br>';
...
$sql1 = mysql_query("SELECT * FROM material_almacen,orden_compras WHERE material_almacen.cod_orden = $cod_orden and orden_compras.cod_orden = $cod_orden")or die(mysql_error());
$array1 = mysql_fetch_array($sql1);
 ?>
<form id="form1" name="form1" class="form-horizontal" enctype="multipart/form-data" method="post" action="" onSubmit="return validar(this)">  
				  <table width="735" height="324" border="0">
                    <tr>
                      <td width="493" height="30"><div class="control-group">
                    ...
                    <input name="cod_material" type="text" class="input-xlarge disabled" id="cod_material" onKeyUp="this.value=this.value.toUpperCase();" size="30" value="<?php echo $array1['cod_material']; ?>" readonly="readonly">
                      ...
                      <td width="10">&nbsp;</td>
                      <td width="10">&nbsp;</td>
                      <td width="204" rowspan="3"><div align="center">
                        <?php
...
?>
               ...
               <input name="material" type="text" id="material" onKeyUp="this.value=this.value.toUpperCase();" size="30" autocomplete="off" value="<?php echo $array1['material']; ?>" pattern="[A-ZÑa-zñ0-9º ]{3,50}" class="input-xlarge disabled" readonly="readonly"/>
                      </div>
                      </div>
					  </div></td>
                      <td>&nbsp;</td>
                    ...
                    <tr>
                      ...
                      <label class="control-label" for="inputWarning">Precio Producto: </label>
                      <div class="controls">
					  <div class="input-prepend" title="Ingrese Precio del Material" data-rel="tooltip">
                     <span class="add-on">$</span><input name="precio_venta" type="text" id="precio_venta" class="input-xlarge disabled" autocomplete="off" value="<?php echo $array1['precio_venta']; ?>" readonly="readonly"/>
                      </div>
                    </div>
					</div></td>
                      <td>&nbsp;</td>
                    ...
                    <tr>
                      <td height="38"><div class="control-group">
                      <label class="control-label" for="inputWarning">Status Producto: </label>
                      <div class="controls">
					  <div class="input-prepend" data-rel="tooltip">
                     <select name="status_material" id="status_material" data-rel="chosen" required="required">
<option value="ACTIVO" <?php if (!(strcmp('ACTIVO', $array1['status_material']))) {echo "selected=\"selected\"";} ?>>ACTIVO</option>
<option value="INACTIVO" <?php if (!(strcmp('INACTIVO', $array1['status_material']))) {echo "selected=\"selected\"";} ?>>INACTIVO</option>
						  </select>
                      </div>
                    </div>
					</div></td>
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
                      <td rowspan="3"><p style="font-size:10px">Para Subir una imagen debe tener en cuenta lo siguiente:</p>
                        <p class="Estilo1">* La Imagen debe ser extension.jpg</p>
                      <p class="Estilo1">* La imagen no debe ser mayor de 200 KB</p></td>
                    </tr>
                    <tr>
                      <td height="30"><div class="control-group">
                      <label class="control-label" for="inputWarning">Existencia Producto: </label>
                      <div class="controls">
					  <div class="input-prepend" title="Escriba solo numeros para Existencia" data-rel="tooltip">
                     <input name="existencia" type="text" class="input-xlarge disabled" id="existencia" value="<?php echo $array1['existencia']; ?>"  pattern="[0-9]{1,5}" placeholder="Ingrese Existencia" readonly="readonly"/>
                      </div>
                    </div>
					</div></td>
                      <td>&nbsp;</td>
                    ...
                    <tr>
                      <td height="30"><div class="control-group">
                      <label class="control-label" for="inputWarning">Minimo Stock: </label>
                      <div class="controls">
					  <div class="input-prepend" title="Escriba solo numeros para Minimo de Stock" data-rel="tooltip">
                     <input name="minimo_stock" type="text" class="input-xlarge focused" id="minimo_stock" value="<?php echo $array1['minimo_stock']; ?>" pattern="[0-9]{1,5}"  autocomplete="off" placeholder="Ingrese Minimo de Stock" required="required"/>
                      </div>
                    </div>
					</div></td>
                      <td>&nbsp;</td>
                    ...
                    <tr>
                      <td height="30"><div class="control-group">
                      <label class="control-label" for="inputWarning">Ubicaciòn Producto: </label>
                      <div class="controls">
					  <div class="input-prepend" title="Ingrese Ubicacion en Deposito" data-rel="tooltip">
                      <input name="ubicacion_deposito" class="input-xlarge focused" value="<?php echo $array1['ubicacion_deposito']; ?>" onKeyUp="this.value=this.value.toUpperCase();" autocomplete="off" id="ubicacion_deposito" size="30" maxlength="80" placeholder="Ingrese Ubicacion en Deposito"   type="text" required="required">
                      </div>
                    </div>
					</div></td>
                      <td>&nbsp;</td>
                      <td>&nbsp;</td>
                      <td rowspan="2"><p align="center" class="Estilo1"><span class="Estilo15">
                        <?php
    //Mostramos la imagen
...
?>
                    ...
                    <tr>
                      ...
                      <div class="controls">
					  <div class="input-prepend" data-rel="tooltip">
					 <input name="rutprov" type="text" id="rutprov" value="<?php $sql=mysql_query("select * from proveedores where rutprov = '".$array1['rutprov']."'") or die (mysql_error()); 
	  $array = mysql_fetch_array($sql);
	  echo $array['nombreprov']; ?>" pattern="[A-ZÑa-zñ0-9º ]{3,50}" class="input-xlarge disabled" readonly="readonly"/> 
                      </div>
                    </div>
					</div></td>
                      <td>&nbsp;</td>
			...
			<input type="file" class="input-file uniform_on" name="imagen" id="imagen"/>
				  ...
				  <div class="form-actions">
                      <button type="submit" class="btn btn-large btn-primary">Actualizar</button>
						...
						<input name="cod_orden" type="hidden" cod_material="cod_orden" value="<?php echo $array1['cod_orden']; ?>" />
			      </div></form>
		        </div>
		      </div><!--/span-->
